...
typedef enum moverMotor{inicio,high,low};
...
moverMotor pasos_1 = inicio;
moverMotor pasos_2 = inicio;
...
long int pasos1=400;
long int pasos2=400;
unsigned long long int tiempo_micros1=0;
unsigned long long int tiempo_micros2=0;
  //Duty cycle 96,15%
void setup() {
...
void moverMotorVelocidad(int pin,moverMotor*pasos_,unsigned long long int*_tiempo_micros){
...
*/
  switch(*pasos_){
    case inicio:{
      *_tiempo_micros=micros();
      *pasos_=high;
    }
    break;
    case high:{
      if(*_tiempo_micros+5000>micros()){
        digitalWrite(pin,HIGH);
      }
      else{
        *_tiempo_micros=0;
      ...
      case MOVIMIENTO:{
        ...
        if(*_tiempo+*_duracion>=millis()){
          if(pin_pasos==STEP_PIN_Y){
            moverMotorVelocidad(STEP_PIN_Y,&pasos_1,&tiempo_micros1);
          } 
          if(pin_pasos==STEP_PIN_X){
            moverMotorVelocidad(STEP_PIN_X,&pasos_2,&tiempo_micros2);
          }
        }
        else{
          *_tiempo=0;
